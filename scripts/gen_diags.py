...
if sys.argv[1] == "h":
    ...
    header_f.write(
        "typedef struct {DiagKind t; union DiagData data; SourcePosition range;} Diag; "
    )
    ...
    for idx, (k, v) in enumerate(diags.items()):
        ...
        for format_spec in v[1]:
            if format_spec.strip() == "%t":
                arg_list += ["Type *type" + str(type_cnt)]
                case_block += (
                    "error_output_type(file, diag->data."
                    + k.strip()
                    + ".type"
                    + str(type_cnt)
                    + ");\n"
                ...
                arg_list += ["SourcePosition pos" + str(pos_cnt)]
                case_block += (
                    "error_output_pos(file, diag->data."
                    + k.strip()
                    + ".pos"
                    + str(pos_cnt)
                    + ");\n"
                ...
                arg_list += ["uint8_t c" + str(char_cnt)]
                case_block += (
                    "error_output_char(file, diag->data."
                    + k.strip()
                    + ".c"
                    + str(char_cnt)
                    + ");\n"
                ...
                arg_list += ["int i" + str(int_cnt)]
                case_block += (
                    "error_output_int(file, diag->data."
                    + k.strip()
                    + ".i"
                    + str(int_cnt)
                    + ");\n"
